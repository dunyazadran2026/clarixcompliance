...
// ── BROWSER TAB ATTENTION TITLE ───────────────
const CX_ORIGINAL_TITLE = document.title || 'Clarix Compliance';
let cxTitleTimer = null;
function startComeBackTitle(){
  const titles = lang === 'da'
    ? ['Kom tilbage — jeg kan hjælpe dig', 'Clarix Compliance · AI Workshops']
    : ['Come back — I can help you', 'Clarix Compliance · AI Workshops'];
  let i = 0;
  document.title = titles[0];
  clearInterval(cxTitleTimer);
  cxTitleTimer = setInterval(() => {
    i = (i + 1) % titles.length;
    document.title = titles[i];
  }, 1800);
}
function stopComeBackTitle(){
  clearInterval(cxTitleTimer);
  cxTitleTimer = null;
  document.title = CX_ORIGINAL_TITLE;
}
document.addEventListener('visibilitychange', () => {
  if (document.hidden) startComeBackTitle();
  else stopComeBackTitle();
});
...
// ── HOME HERO SLIDER ──────────────────────────
let cxHeroSliderTimer = null;
function initHeroSlider(){
  const slides = Array.from(document.querySelectorAll('.cx-hero-slide'));
  const dots = Array.from(document.querySelectorAll('.cx-hero-dot'));
  ...
  clearInterval(cxHeroSliderTimer);
  dots.forEach((dot, i) => dot.onclick = () => show(i));
  show(0);
  cxHeroSliderTimer = setInterval(() => show((index + 1) % slides.length), 5200);
}
...
function buildHome(){
  ...
  const heroSlides = lang === 'da' ? [
    {k:'AI WORKSHOPS', h:'AI-workshops for RA/QA og ledelse', p:'Praktiske forløb med AI som assisterende værktøj, human review og audit-ready governance.', img:'AI workflow lab', cta:'Se AI Workshops', target:'ai'},
    {k:'REGULATORY WORK', h:'AI til regulatorisk arbejde', p:'Brug AI til struktur, udkast, research og gap-analyse — med dokumenteret menneskelig verifikation.', img:'Regulatory workspace', cta:'Udforsk services', target:'devices'},
    {k:'QUALITY SYSTEMS', h:'Mere struktur i QMS-arbejdet', p:'Fra SOPer og CAPA til change control: bedre workflows uden at miste compliance-kontrollen.', img:'Quality system map', cta:'Se kvalitet', target:'devices'},
    {k:'AI ACT', h:'Governance før implementering', p:'EU AI Act, risikoklassifikation, transparens, datakvalitet og ansvarlig brug i regulerede miljøer.', img:'AI governance cockpit', cta:'Læs mere', target:'ai'}
  ] : [
    {k:'AI WORKSHOPS', h:'AI workshops for RA/QA and leadership', p:'Practical sessions with AI as an assistive tool, human review and audit-ready governance.', img:'AI workflow lab', cta:'View AI Workshops', target:'ai'},
    {k:'REGULATORY WORK', h:'AI for regulatory work', p:'Use AI for structure, drafting, research and gap analysis — with documented human verification.', img:'Regulatory workspace', cta:'Explore services', target:'devices'},
    {k:'QUALITY SYSTEMS', h:'More structure in QMS work', p:'From SOPs and CAPA to change control: better workflows without losing compliance control.', img:'Quality system map', cta:'View quality', target:'devices'},
    {k:'AI ACT', h:'Governance before implementation', p:'EU AI Act, risk classification, transparency, data quality and responsible use in regulated environments.', img:'AI governance cockpit', cta:'Read more', target:'ai'}
  ];
  ...
  <section class="hero">
    ...
    <div class="w hero-inner">
      <div class="cx-hero-slider-wrap">
        <div class="hero-label">${t('hero_ey')}</div>
        <div class="cx-hero-slider" id="home-hero-slider">
          ${heroSlides.map((s,i)=>`
            <article class="cx-hero-slide ${i===0?'active':''}">
              <div class="cx-slide-copy">
                <span class="cx-slide-kicker">${s.k}</span>
                <h1>${s.h}</h1>
                <p class="hero-sub">${s.p}</p>
                <div class="hero-actions">
                  <button class="btn btn-teal" onclick="showPage('${s.target}')">${s.cta}</button>
                  <button class="btn btn-ghost" onclick="showPage('contact')">${t('book_nav')}</button>
                </div>
              </div>
              <div class="cx-slide-visual" aria-label="${s.img}">
                <div class="cx-visual-orb"></div>
                <div class="cx-visual-card cx-vc-1">AI</div>
                <div class="cx-visual-card cx-vc-2">QMS</div>
                <div class="cx-visual-card cx-vc-3">RA</div>
                <span>${s.img}</span>
              </div>
            </article>
          `).join('')}
        ...
        <div class="cx-hero-dots">
          ${heroSlides.map((_,i)=>`<button class="cx-hero-dot ${i===0?'active':''}" aria-label="Slide ${i+1}"></button>`).join('')}
        </div>
      </div>
      <div class="book-card">
        <h3>${t('bk_h')}</h3>
...
function buildAI(){
  ...
  const leadTopics_da = [
    ...
    'AI til personaleledelse og kommunikation — medarbejderudvikling og onboarding',
  ...
  const contactEmail = 'dunya.zadran@outlook.dk';
  const mailto_comp = `mailto:${contactEmail}?subject=Compliance%20AI%20Workshop&body=`+encodeURIComponent('Oplysninger ved tilmelding:

• Navn på deltagere
• Email på deltagere
• Fakturerings-email
• CVR nummer');
  const mailto_lead = `mailto:${contactEmail}?subject=Ledelses%20AI%20Workshop&body=`+encodeURIComponent('Oplysninger ved tilmelding:

• Navn på deltagere
• Email på deltagere
• Fakturerings-email
• CVR nummer');
  const topicCards = (items) => items.map((tp,i)=>`
  ...
  const locCard = (city, date, venue, addr) => `
    <div class="ai-info-card">
      <div class="ai-info-city">${city}</div>
      <div class="ai-info-date">📅 ${date}</div>
      <div class="ai-info-text">${venue}</div>
      <div class="ai-info-text small">${addr}</div>
    </div>`;
  document.getElementById('page-ai').innerHTML =
    pageHero(
      ...
      L==='da'
        ...
        : 'Two specialised workshops delivered by Clarix Compliance — one for RA/QA professionals in regulated environments and one for leaders and independents. Both are practical, compliance-oriented and directly applicable from day one.',
      'ph-ai'
    )
    +
    `
    <section class="ai-workshop-nav">
      <div class="w">
        <div class="ai-workshop-nav-inner">
          <button class="ai-workshop-tab active" data-workshop="comp-workshop" onclick="showAIWorkshop('comp-workshop')">Compliance AI Workshop</button>
          <button class="ai-workshop-tab" data-workshop="lead-workshop" onclick="showAIWorkshop('lead-workshop')">${L==='da'?'Ledelses AI Workshop':'Leadership AI Workshop'}</button>
        </div>
      </div>
    </section>
    <section class="sec sec-white ai-workshop-stage">
      <div class="w">
        <div id="comp-workshop" class="ai-workshop-panel active" style="display:block;">
          <div class="ai-workshop-header-grid">
            <div>
              <span class="eyebrow">Workshop 01</span>
              <h2 class="sec-title">Compliance AI Workshop</h2>
              <p class="sec-sub">${L==='da'?'En heldags workshop for RA- og QA-fagfolk i regulerede miljøer, der ønsker at anvende AI struktureret, dokumenteret og compliance-sikkert.':'A full-day workshop for RA and QA professionals in regulated environments who want to use AI in a structured, documented and compliance-safe manner.'}</p>
            </div>
            <div class="ai-video-placeholder">
              <div class="ai-play">▶</div>
              <span>${L==='da'?'Video-placeholder: introduktion til Compliance AI Workshop':'Video placeholder: intro to Compliance AI Workshop'}</span>
            </div>
          </div>
          <div class="ai-visual-row">
            ${locCard(L==='da'?'København':'Copenhagen','10. juni 2026','Health Tech Hub Copenhagen','Danneskiold-Samsøes Allé 41, 1434 København')}
            ${locCard('Aarhus','26. august 2026','Kitchen Aarhus','Universitetsbyen 76, 8000 Aarhus')}
            ${locCard('Odense','10. september 2026','Forskerparken','Forskerparken 10, 5230 Odense')}
            <div class="ai-info-card price"><div class="ai-info-city">${L==='da'?'Pris':'Price'}</div><div class="ai-info-date">4.475 kr. + moms</div><div class="ai-info-text">${L==='da'?'Partnerpris fra 2.475 kr.':'Partner price from 2,475 DKK'}</div></div>
          </div>
          <div class="ai-workshop-grid">${topicCards(compTopics)}</div>
          <div class="ai-actions"><a href="${mailto_comp}" class="btn btn-navy">${L==='da'?'📅 Tilmeld workshop':'📅 Register for workshop'}</a><button class="btn btn-outline" onclick="showPage('contact')">${L==='da'?'Spørg om workshop':'Ask about workshop'}</button></div>
        </div>
        <div id="lead-workshop" class="ai-workshop-panel" style="display:none;">
          <div class="ai-workshop-header-grid">
            <div>
              <span class="eyebrow">Workshop 02</span>
              <h2 class="sec-title">${L==='da'?'Ledelses AI Workshop':'Leadership AI Workshop'}</h2>
              <p class="sec-sub">${L==='da'?'En specialiseret heldags workshop for ledere, projektledere og selvstændige, der ønsker at anvende AI struktureret og strategisk i praksis.':'A specialised full-day workshop for leaders, project managers and independents who want to use AI in a structured and strategic manner.'}</p>
            </div>
            <div class="ai-video-placeholder leadership">
              <div class="ai-play">▶</div>
              <span>${L==='da'?'Video-placeholder: AI som executive assistant':'Video placeholder: AI as executive assistant'}</span>
            </div>
          </div>
          <div class="ai-visual-row">
            ${locCard('Odense','3. september 2026','Forskerparken','Forskerparken 10, 5230 Odense')}
            <div class="ai-info-card"><div class="ai-info-city">${L==='da'?'Tilmeldingsfrist':'Registration deadline'}</div><div class="ai-info-date">24. august 2026</div><div class="ai-info-text">${L==='da'?'50% ved afbud efter 3. august':'50% cancellation fee after Aug 3'}</div></div>
            <div class="ai-info-card price"><div class="ai-info-city">${L==='da'?'Standardpris':'Standard price'}</div><div class="ai-info-date">6.500 kr. + moms</div><div class="ai-info-text">Early Bird: 5.500 kr.</div></div>
            <div class="ai-info-card price"><div class="ai-info-city">${L==='da'?'Partnerpris':'Partner price'}</div><div class="ai-info-date">4.500 kr. + moms</div><div class="ai-info-text">${L==='da'?'Erhvervshus Fyn & Forskerparken':'Erhvervshus Fyn & Forskerparken'}</div></div>
          </div>
          <div class="ai-workshop-grid">${topicCards(leadTopics)}</div>
          <div class="ai-actions"><a href="${mailto_lead}" class="btn btn-navy">${L==='da'?'📅 Tilmeld workshop':'📅 Register for workshop'}</a><button class="btn btn-outline" onclick="showPage('contact')">${L==='da'?'Spørg om workshop':'Ask about workshop'}</button></div>
        </div>
      </div>
    </section>
    `
    + pageCTA(
      ...
      L==='da'?'Book en plads på en workshop — eller kontakt mig for et skræddersyet forløb til din organisation.':'Book a seat at a workshop — or contact me for a tailored programme for your organisation.',
      'ph-ai-cta'
    );
...
function submitForm(btn){
  const wrap = btn.closest('.cf-wrap');
  const inputs = wrap ? wrap.querySelectorAll('input, select, textarea') : [];
  const first = inputs[0]?.value || '';
  const last = inputs[1]?.value || '';
  const email = inputs[2]?.value || '';
  const company = inputs[3]?.value || '';
  const topic = inputs[4]?.value || '';
  const message = inputs[5]?.value || '';
  const subject = encodeURIComponent('Ny henvendelse fra clarixcompliance.com');
  const body = encodeURIComponent(`Navn: ${first} ${last}
Email: ${email}
Virksomhed: ${company}
Område: ${topic}
