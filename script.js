...
function showAIWorkshop(id) {
  ...
  document.querySelectorAll('.ai-workshop-tab').forEach(tab => {
    const isActive = tab.getAttribute('data-workshop') === id;
    tab.classList.toggle('active', isActive);
    tab.style.background = isActive ? '#163A5F' : '#FFFFFF';
    tab.style.color = isActive ? '#FFFFFF' : '#163A5F';
    tab.style.borderColor = isActive ? '#163A5F' : 'rgba(22,58,95,.14)';
  });
...
function buildAI(){
  ...
  const mailtoComp = 'mailto:dunya.zadran@outlook.dk?subject=Compliance%20AI%20Workshop&body=' + encodeURIComponent(
    'Oplysninger der skal oplyses ved tilmelding:
...
• CVR nummer'
  ...
  const mailtoLead = 'mailto:dunya.zadran@outlook.dk?subject=Ledelses%20AI%20Workshop&body=' + encodeURIComponent(
    'Oplysninger der skal oplyses ved tilmelding:
...
• CVR nummer'
  ...
  const workshopCard = (num, title, text) => `
    <div style="background:#FFFFFF;border:1px solid rgba(22,58,95,.08);border-left:3px solid #1C8C8C;border-radius:2px;padding:22px;min-height:150px;">
      <span class="eyebrow" style="margin-bottom:8px;">${num}</span>
      <h3 style="font-family:'Outfit',sans-serif;font-weight:400;font-size:16px;color:#163A5F;margin:0 0 8px;">${title}</h3>
      <p style="font-size:12px;font-weight:300;color:#8B9BB4;line-height:1.7;margin:0;">${text}</p>
    </div>`;
  ...
  document.getElementById('page-ai').innerHTML =
    ...
    +
    `
    <section class="ai-workshop-nav" style="background:#F5F7FA;padding:28px 0;">
      <div class="w">
        <div style="display:flex;gap:12px;flex-wrap:wrap;background:#FFFFFF;border:1px solid rgba(22,58,95,.08);padding:18px;border-radius:2px;">
          <button
            class="ai-workshop-tab active"
            data-workshop="workshop-ai-workflows"
            onclick="showAIWorkshop('workshop-ai-workflows')"
            style="background:#163A5F;color:#FFFFFF;border:1px solid #163A5F;padding:12px 18px;border-radius:2px;font-family:'Inter',sans-serif;font-size:12px;font-weight:500;cursor:pointer;">
            ${L==='da' ? 'Compliance AI Workshop' : 'Compliance AI Workshop'}
          </button>

          <button
            class="ai-workshop-tab"
            data-workshop="workshop-ai-leadership"
            onclick="showAIWorkshop('workshop-ai-leadership')"
            style="background:#FFFFFF;color:#163A5F;border:1px solid rgba(22,58,95,.14);padding:12px 18px;border-radius:2px;font-family:'Inter',sans-serif;font-size:12px;font-weight:500;cursor:pointer;">
            ${L==='da' ? 'Ledelses AI Workshop' : 'Leadership AI Workshop'}
          </button>
        </div>
      </div>
    </section>
    ...
    <section class="sec sec-white">
      <div class="w">
        ...
        <div id="workshop-ai-workflows" class="ai-workshop-panel" style="display:block;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:28px;align-items:start;margin-bottom:32px;">
            <div>
              <span class="eyebrow">Workshop 01</span>
              <h2 class="sec-title">Compliance AI Workshop</h2>
              <p class="sec-sub" style="max-width:100%;">
                ${L==='da'
                  ? 'En heldags workshop for RA- og QA-fagfolk i regulerede miljøer, der ønsker at anvende AI struktureret, dokumenteret og compliance-sikkert.'
                  : 'A full-day workshop for RA and QA professionals in regulated environments who want to use AI in a structured, documented and compliance-safe manner.'}
              </p>
            </div>

            <div style="background:#F5F7FA;border:1px solid rgba(22,58,95,.08);border-radius:2px;padding:26px;">
              <span class="eyebrow">${L==='da'?'Praktisk information':'Practical information'}</span>
              <p style="font-size:13px;font-weight:300;color:#8B9BB4;line-height:1.7;margin:10px 0 0;">
                09:00–15:00 · ${L==='da'?'Heldags workshop':'Full-day workshop'} · RA/QA · MDR · IVDR · EU AI Act
              </p>
              <p style="font-size:13px;font-weight:500;color:#163A5F;margin:12px 0 0;">
                4.475 kr. + moms · ${L==='da'?'Partnerpris fra 2.475 kr.':'Partner price from 2,475 DKK'}
              </p>
            </div>
          </div>

          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-bottom:28px;">
            ${workshopCard('01', L==='da'?'AI som assisterende værktøj':'AI as an assistive tool', L==='da'?'Brug AI til udkast, strukturering, research, gap-analyser og dokumentarbejde — aldrig som endelig sandhed.' : 'Use AI for drafts, structuring, research, gap analysis and documentation work — never as final truth.')}
            ${workshopCard('02', 'Human review & verification', L==='da'?'Design review-punkter, verifikation, dokumentation og menneskelig kontrol i regulerede workflows.' : 'Design review points, verification, documentation and human oversight in regulated workflows.')}
            ${workshopCard('03', 'QMS integration', L==='da'?'Kobl AI workflows til SOPer, change control, dokumentstyring, traceability og audit trail.' : 'Connect AI workflows to SOPs, change control, document control, traceability and audit trail.')}
          </div>

          <div style="display:flex;gap:12px;flex-wrap:wrap;">
            <a href="${mailtoComp}" class="btn btn-navy">${L==='da'?'📅 Tilmeld workshop':'📅 Register for workshop'}</a>
            <button class="btn btn-outline" onclick="showPage('contact')">${L==='da'?'Spørg om workshop':'Ask about workshop'}</button>
          </div>
        </div>

        <div id="workshop-ai-leadership" class="ai-workshop-panel" style="display:none;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:28px;align-items:start;margin-bottom:32px;">
            <div>
              ...
              <p class="sec-sub" style="max-width:100%;">
                ${L==='da'
                  ? 'En specialiseret heldags workshop for ledere, projektledere og selvstændige, der ønsker at anvende AI struktureret og strategisk i praksis.'
                  : 'A specialised full-day workshop for leaders, project managers and independents who want to use AI in a structured and strategic manner.'}
              </p>
            </div>
            ...
            <div style="background:#F5F7FA;border:1px solid rgba(22,58,95,.08);border-radius:2px;padding:26px;">
              <span class="eyebrow">${L==='da'?'Praktisk information':'Practical information'}</span>
              <p style="font-size:13px;font-weight:300;color:#8B9BB4;line-height:1.7;margin:10px 0 0;">
                Odense · 09:00–15:00 · ${L==='da'?'Ledere & selvstændige':'Leaders & independents'}
              </p>
              <p style="font-size:13px;font-weight:500;color:#163A5F;margin:12px 0 0;">
                6.500 kr. + moms · Early Bird 5.500 kr. · ${L==='da'?'Partnerpris 4.500 kr.':'Partner price 4,500 DKK'}
              </p>
            </div>
          </div>

          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-bottom:28px;">
            ${workshopCard('01', L==='da'?'AI som executive assistant':'AI as executive assistant', L==='da'?'Brug AI til email, møder, planlægning, prioritering, analyse og produktivitet.' : 'Use AI for email, meetings, planning, prioritisation, analysis and productivity.')}
            ${workshopCard('02', L==='da'?'Strategi og beslutninger':'Strategy and decisions', L==='da'?'Brug AI som støtte til strategi, risikovurdering, forretningsudvikling og bedre beslutningsprocesser.' : 'Use AI to support strategy, risk assessment, business development and better decision-making.')}
            ${workshopCard('03', 'Responsible AI governance', L==='da'?'Forstå governance, ansvar, datasensitivitet, human oversight og praktiske guardrails.' : 'Understand governance, accountability, data sensitivity, human oversight and practical guardrails.')}
          </div>

          <div style="display:flex;gap:12px;flex-wrap:wrap;">
            <a href="${mailtoLead}" class="btn btn-navy">${L==='da'?'📅 Tilmeld workshop':'📅 Register for workshop'}</a>
            <button class="btn btn-outline" onclick="showPage('contact')">${L==='da'?'Spørg om workshop':'Ask about workshop'}</button>
          </div>
        </div>
